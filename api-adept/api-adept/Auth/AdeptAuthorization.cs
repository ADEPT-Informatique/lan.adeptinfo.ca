...
namespace api_adept.Auth
{
    ...
    public class GenericAuthenticationFilter : AuthorizationFilterAttribute
    {
        ...
        /// <param name="filterContext"></param>  
        /// <returns></returns>  
        protected virtual bool OnAuthorizeUser(string user, string email, HttpActionContext filterContext)
        {
            if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(email)) return false;
            return true;
        }
